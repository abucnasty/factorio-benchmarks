...
ticks_total="3600"
# results folder
results_folder="collector_placement_results"
# output folder
output_folder="collector_placement_charts"
# aggregation strategy
aggregate_strategy="average"
# standard deviation filter of runs
stdev_filter="2"

belt-charts boxplot "$results_folder/prom_side_collectors_*.csv" \
    -w 1000 \
    -h 800 \
    --remove-first-ticks $ticks_trim \
    --trim-prefix "prom_side_collectors_" \
...
belt-charts summary "$results_folder/prom_side_collectors_*.csv" \
    -w 1200 \
    -h 700 \
    --remove-first-ticks $ticks_trim \
...
belt-charts bar "$results_folder/prom_side_collectors_*.csv" \
    -w 1200 \
    -h 800 \
    --remove-first-ticks $ticks_trim \
    --metrics "wholeUpdate,controlBehaviorUpdate,transportLinesUpdate,electricHeatFluidCircuitUpdate,entityUpdate,particleUpdate,turretTargetAcquisition,spacePlatforms" \
...
belt-charts entity-summary "$results_folder/prom_side_collectors_*.csv" \
    --remove-first-ticks $ticks_trim \
...
belt-charts entity-summary-per-run "$results_folder/prom_side_collectors_*.csv" \
    -w 2000 \
    -h 800 \
    --remove-first-ticks $ticks_trim \
...
belt-charts entity-matrix "$results_folder/prom_side_collectors_*.csv" \
    -w 1200 \
    --remove-first-ticks $ticks_trim \
    --stddev-filter "$stdev_filter" \
    --aggregate-file "$results_folder/results.csv" \
